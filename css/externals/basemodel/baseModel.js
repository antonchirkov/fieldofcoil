...
(function() {
	...
	modelNS.BaseModel = Backbone.Model.extend({
		...
		initialize: function(options) {
			...
			if (this.restyling) {
				...
				if (!this.height) {
					this.height = this.options.height || this.wrapper.height();
				}
			}
		},
	...
	modelNS.BaseModelView = modelNS.View.extend({
		...
		renderOnShow: function () {
			var $wrapper = this.model.wrapper,
				...
				$popup = $wrapper.parents(".ui-dialog-content"),
				self = this;
            /*
             * Если модель находится внутри ВО, то сначала дожидаемся его открытия.
             * Получаем экземпляр виджета ВО. Данный экземпляр нужен для надёжной проверки, что ВО открыто.
             */
			if ($popup.length !== 0) {
				var dialogInstance = $popup.popup('instance');
				if (dialogInstance.isPopupOpened()) {
                    self.renderNowOrWhenTabActivated({$wrapper, $tabs});
				} else {
        ...
        /**
		 * Если модель находится в неактивном табе, то регистрируется событие, по которому модель будет рендериться
		 * после перехода на таб, на котором она находится.
		 * В остальных случаях модель рендерится сразу.
         * @param $wrapper {jQuery} Объект, представляющий обёртку модели .model-wrp.
         * @param $tabs {jQuery} Объект, представляющий элемент с табами.
         */
        renderNowOrWhenTabActivated: function({$wrapper, $tabs}) {
			var self = this;
	...
	modelNS.HorizontalSlider = Backbone.View.extend({
		...
		render: function() {
			...
			var columns = $parent.attr('columns')*1 || 1;
			var width = $parent.width() / columns - 40;
		...
		calculateBackground: function(ui) {
			...
			if (this.model.get('scale') == 'linear' || this.model.get('scale') == 'logarithmic') {
				...
				max = Math.floor(max / this.model.get('range')) * this.model.get('range') + 1;
				var pixelsPerMark = this.slider.width() / (max - min - 1),
					width = (ui.value - min) * pixelsPerMark;
				this.sliderBack.width(width);
			}
		},
	...
	modelNS.Input = Backbone.View.extend({
		...
		validate: function (val, defVal) {
			...
			if (this.inputType == 'number') {
                ...
                numVal = Math.getClosestMultiple(numVal, this.step);
...
modelNS.ColorSlider = Backbone.View.extend({
    ...
    render: function () {
        ...
        this.slider.slider({
            ...
            slide: function (event, ui) {
                ...
                _this.colorPicker.css('background-color', _this.waveLengthToRgb(ui.value))
            },
        })
        ...
        if (this.label) {
            this.slider.before('<div class="color-slider-label">' + this.label + '</div>');
		}
    ...
    /**
	 * Возвращает строку, представляющую цвет, в зависимости от значения, которое передано в аргументе value.
	 * Если в аргументе value ничего не было передано, то получаем то значение, которое на данный момент имеет слайдер.
	 * Если было передано значение, то для получения цвета используется именно это значение.
     * @param {number|null} value Значение для получения цвета
     * @returns {string} Строка, представляющая цвет
     */
    getCurrentColor: function (value = null) {
    	value = value === null ? this.getValue() : value;
        return this.waveLengthToRgb(value);
    },
});
...
/* ---------------- NUMBERS --------------- */
modelNS.parseFloat = Math.parseFloat;
