...
// console.log(type.activeRadioID, radio.value, self)
	...
	renderTypeLayers: function (type) {
		...
		var interval = setInterval(function() {
			...
			// добавление содержимого скрипта <svg>
			var correctXML = layer.src.replace(/\"\&/gi,'"&amp;'),
					$svgsrc = $('<div/>'), // $(correctXML), // svg код без всякой обработки от jquery #10679-6
					$svg;

			// svg код без всякой обработки от jquery #10679-6
			$svgsrc[0].innerHTML = correctXML;

			$svgsrc.find('[title]').attr('title', ''); // clear default titles

			if (type.layers.radio) {	// #9483
				$typeWrp.prepend($svgsrc);	// #9249
				$svg = $typeWrp.find('svg').first();
			} else {
				$typeWrp.append($svgsrc);
				$svg = $typeWrp.find('svg').last();
			}

			$svg.attr('id', layer.id);	// #10716 id должен быть присвоен svg
			...
			if (type.layers.radio) {
				if (type.activeRadioID != layer.id) $svg.remove();
	...
	switchType: function (typeID, options) {
		...
		if ($typeWrp.attr('multimedia') === 'true') {
			this.$multimediaBtn.show();
		} else {
			this.$multimediaBtn.hide();
		}
		if ($typeWrp.attr('backbutton') === 'true') {
