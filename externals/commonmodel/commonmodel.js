...
modelNS.CommonModel = modelNS.BaseModel.extend({
	...
	initialize: function(options) {
		...
		$.when(modelsDeferred, commonDeferred)
			.then(function () {
				courseML.addModuleanswer(new modelNS.CommonModel.models[type](options.xmlData, options.wrapper, options.basePath, options.params).init(), options.params);
			});
	},
