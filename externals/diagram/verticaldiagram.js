...
// console.log(value, [valueWidth, diffLeft], [left, valueRight])
			...
			this.$label.css('bottom', this.bottomOutset + 4); // 4 отступ от шкалы
		},
	...
	iDiagramNS.VerticalScale = Backbone.View.extend({
		...
		renderScale: function() {
			...
			for (var i = 0; i < this.axisData.length; i++) {
				...
				if (isShowMarks) {
					var markVal = modelNS.valueToLabel(this.axisData.at(i).get('value'));
					...
					$mark.css({
							top: ((this.axisData.length - 1 - (i + (this.scaleCentered ? 0 : 0.5))) * this.pixelsPerMark),
							marginTop: (this.pixelsPerMark-$mark.height())/2
					});
		...
		setBottomOutset: function (bottomOutset) {
			...
			if (this.max === undefined || this.min === undefined) {
				...
				this.pixelsPerMark = height / (Math.round((this.max - this.min) / this.step));
				this.pixelsPerStep = height / (this.max - this.min);
			}
