(function() {
	...
	iDiagramNS.TableCell = Backbone.View.extend({
		...
		initialize: function(options) {
			...
			this.parent = options.parent;
			this.value = isNaN(options.value) ? options.value : this.numberDivide(options.value); // ##12701 Если пришло число, то поделим его на разряды
