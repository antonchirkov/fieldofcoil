...
// console.log(courseML.getHTMLFromCourseML($xml.find('td')[0]))
      ...
      if (from) {
        dataBlock = $('<div/>').html(localParam[from]);
      }
      ...
      settings.databutton = true; // always show databutton
			...
			settings.axisXValues = new Backbone.Collection();
			var categories = new iDiagramNS.CategoryCollection();
			dataBlock.first().find('th').each(function(i) {
				if (i == 0) {
          settings.axisTitle = courseML.getHTMLFromCourseML($(this)); // $(this).text();
					return;
				}
				settings.axisXValues.add({value: courseML.getHTMLFromCourseML($(this))}); // $(this).text()
			});
			...
			dataBlock.first().find('tr').each(function(i) {
				...
				var points = new iDiagramNS.PointCollection();
				    color = $(this).attr('color') || iDiagramNS.DEFAULT_COLORS[i - 1],
					title = '';
				var c = new iDiagramNS.Category({
						color: color
					});
				$(this).find('td').each(function(j) {
					if (j == 0) {
						title = courseML.getHTMLFromCourseML($(this)); // $(this).text();
						return;
					}

					points.add({
						category: c,
						value: courseML.getHTMLFromCourseML($(this)), //.text(),
						axisValue: settings.axisXValues.at(j - 1),
						link: $(this).attr('link')
					});
				});
				c.set({title: title, points: points});
				categories.add(c);
			});
...
modelNS.IDiagramView = modelNS.BaseModelView.extend({
  ...
  // расчитать значения шкалы
    ...
    if (!this.step) {
      ...
      if (D<=1) {
		...
		i = 0;
		while (((this.max - this.min) / this.step > 20) || i == 200) {
			this.step = this.step * 2;
			i++;
		}

    // если изменить данные должно пересчитаться
		// this.model.dataJSON.min = this.min;
		// this.model.dataJSON.max = this.max;
		// this.model.dataJSON.step = this.step;
	},
  ...
  renderPetalDiagram: function() {
		this.petalsDiagram = new iDiagramNS.PetalsDiagram(this.diagramOptions);
