			...
			this.options.categories.each(function(c, i) {
				...
				c.get('points').each(function(p, k) {
					...
					if (axis && value) {
						...
						// #11386 Если указаны римские цифры, то переводим в числа
						x = isNaN(x) ? self.romanToArabConvert(axis) : x;
	          y = isNaN(y) ? self.romanToArabConvert(value) : y;
					}

					points.push({
						value: y,
						left: self.hScale.getScaledLeft(x),
						top: self.zeroTop - pixelsPerStep * y, // + 10,
						link: p.get('link'),
					});
				});
